...
import Maintenance from './pages/client/Maintenance'; // <-- IMPORT NEW PAGE
...
const GlobalGuard = ({ children }) => {
    ...
    // If maintenance turns ON, redirect to /maintenance page
    if (maintenance?.active && location.pathname !== '/maintenance') {
        return <Navigate to="/maintenance" replace />;
    }

    // If maintenance turns OFF, but user is stuck on /maintenance, send them home
    if (!maintenance?.active && location.pathname === '/maintenance') {
    ...
    return (
        <GlobalGuard>
            <Routes>
                ...
                {/* NEW: Dedicated Maintenance Route */}
